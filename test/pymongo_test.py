...
def readData(filename):
    data = read_csv(filename, sep=",", nrows=1000000)
...
# Approach 3, sorting list before upload AND splitting upload into chunks:
# Approach 2 was relatively much faster than #1, but for >1M rows the upload time
# was extremely long. I do not know if this has to do with my computer or perhaps docker.
tagListOrder = []
tagListData = []
startTime = tm.time()
print("Splitting into lists corr. to tagID...")
...
for i in range(len(tagListOrder)):
    collection = db[str(tagListOrder[i])]
    try:
        elements = len(tagListData[i])
        iter = 0
        while elements-iter>1000:
            result = collection.insert_many(tagListData[i][iter:iter+999])
            iter += 1000
        result = collection.insert_many(tagListData[i][iter:elements-1])
        print("Tag " + str(i) + " uploaded. " + str(len(tagListData[i])) + " rows added.")
    except:
        print("Failed: ", i)
        failed.append(i)
print("Uploaded " + str(len(FA)) + " records in " + str(tm.time()-startTime) + "s.")
